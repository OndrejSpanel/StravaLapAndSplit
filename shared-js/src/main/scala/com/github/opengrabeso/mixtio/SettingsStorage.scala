...
@SerialVersionUID(12)
case class SettingsStorage(questTimeOffset: Int = 0, maxHR: Int = 220, elevFilter: Int = 2, darkSky: Boolean = true) {
  ...
  def setDarkSky(v: Option[Boolean]) = v.map(v => copy(darkSky = v)).getOrElse(this)
}
